...
def skew_v2(w):
    ...
    if w.shape==(n,m,3,3):
        ...
        zero1 = torch.zeros(n, m, 1, 1).to(device)
        w = w.unsqueeze(-1).unsqueeze(-1)
...
def skrew_bracket(V):
    ...
    out = 0
    if V.shape == (n,4,4):
        out = torch.cat([-V[:,1,2].unsqueeze(-1),V[:,0,2].unsqueeze(-1),-V[:,0,1].unsqueeze(-1),V[:,:3,3]],dim=1)
    else:
        W = skew(V[:,0:3])
        out = torch.cat([torch.cat([W,V[:,3:].unsqueeze(-1)],dim=2),torch.zeros(n,1,4,device=device)],dim=1)
...
def Dexp_so3(w):
    R = exp_so3(w)
    N = w.shape[0]
    Id = torch.eye(3).to(device)
    dRdw = torch.zeros(N, 3, 3, 3).to(device)
    ...
    temp2 = torch.einsum('njk,nki->nij', W, (-R + torch.eye(3).to(device)).to(device))
...
def log_SO3(R):
    ...
    omega = torch.zeros(batch_size, 3, 3).to(device)
    theta = torch.acos((trace - 1)/2)
    temp = theta.unsqueeze(-1).unsqueeze(-1)
    omega[(torch.abs(trace + 1) > eps) * (theta>eps)] = ((temp/(2*torch.sin(temp)))*(R-R.transpose(1,2)))[(torch.abs(trace + 1)>eps) * (theta>eps)]

    omega_temp = (R[torch.abs(trace + 1) <= eps]-torch.eye(3).to(device))/2
    omega_vector = torch.sqrt(omega_temp[:, range(3), range(3)] + torch.ones(3, device=device))
...
def log_SO3_v2(R):
    ...
    omega = torch.zeros(batch_size1, batch_size2, 3, 3).to(device)
    theta = torch.acos((trace - 1)/2)
    temp = theta.unsqueeze(-1).unsqueeze(-1)
    omega[(torch.abs(trace + 1) > eps) * (theta>eps)] = ((temp/(2*torch.sin(temp)))*(R-R.transpose(2,3)))[(torch.abs(trace + 1)>eps) * (theta>eps)]

    omega_temp = (R[torch.abs(trace + 1) <= eps] - torch.eye(3).to(device).unsqueeze(0).unsqueeze(0))/2
    omega_vector = torch.sqrt(omega_temp[:, :, range(3), range(3)] + torch.ones(3, device=device).unsqueeze(0).unsqueeze(0))
    omega_vector[:, :, 1] *= torch.sign(omega_temp[:, :, 0, 1])
    omega_vector[:, :, 2] *= torch.sign(omega_temp[:, :, 0, 2])
...
def exp_so3_T(S):
    ...
    eps = 1e-014
    W = skew(w)
    T = torch.cat([torch.cat([exp_so3(w),v],dim=2),(torch.zeros(n,1,4,device=device))],dim=1)
    T[:,-1,-1] = 1
    return T
...
def inverse_SE3(T):
    ...
    T_inv = torch.cat([torch.cat([R.transpose(1,2),(-R.transpose(1,2))@p],dim=2),torch.zeros(n,1,4,device=device)],dim=1)
    T_inv[:,-1,-1] = 1
...
def large_Ad(T):
    ...
    AdT = torch.cat([torch.cat([R,torch.zeros(n,3,3,device=device)],dim=2),torch.cat([skew(p)@R,R],dim=2)],dim=1)
...
def small_ad(V):
    ...
    wskew = skew(w)
    vskew = skew(v)
    adV = torch.cat([torch.cat([wskew,torch.zeros(n,3,3,device=device)],dim=2),torch.cat([vskew,wskew],dim=2)],dim=1)
...
def log_SO3_T(T):
    ...
    P = torch.eye(3,device=device) + (1-cw)*(wnorm_inv**2)*W + (wnorm_unsqueezed - sw) * (wnorm_inv**3) * torch.matmul(W,W) #n,3,3
    v = p #n,3,1
    return torch.cat([torch.cat([W,v],dim=2),torch.zeros(n,1,4,device=device)],dim=1) #n,4,4
...
def log_SE3(T):
    ...
    return torch.cat([torch.cat([W,v],dim=2),torch.zeros(n,1,4,device=device)],dim=1)
...
def Uniform_sampling(qtraj, batch_size):
    ...
    random_R = torch.tensor(SO3_sampler.rvs(batch_size),dtype=dtype,device=device)
...
#     Gaussian_w = torch.empty(batch_size,3).normal_(mean=0,std=w_std).to(device)
#     Gaussian_p = torch.empty(batch_size,3,1).normal_(mean=0,std=p_std).to(device)
    Gaussian_w = w_distribution.sample((batch_size,)).to(device)
    Gaussian_p = p_distribution.sample((batch_size,)).unsqueeze(-1).to(device)
    ...
    random_T = torch.cat([torch.cat([R_samples,p_samples],dim=2),torch.zeros(batch_size,1,4,device=device)],dim=1).detach()
    #random_T.requires_grad=True
    random_q = skrew_bracket(log_SO3_T(random_T))
...
def SE3_spline(xpoints,Vpoints, T, delta, num_timesteps):
    ...
    ww = torch.from_numpy(np.vstack([ww1,ww2,ww3])).T.to(device)
    ww = ww.type(dtype)
    R_pre = exp_so3(ww[:-1])
    R_cur = exp_so3(ww[1:])
    dw_temp = skew(log_SO3(R_pre.transpose(1,2)@R_cur)/dt)
    dw_f = torch.cat([dw_temp,torch.zeros(1,3).to(device)],dim=0)
    #print(dw_f)
    ...
    #### 5. xtraj and x_dot
    skrew_traj = torch.zeros(num_timesteps,6).to(device)
    twist_traj = torch.zeros(num_timesteps,6).to(device)


    skrew_traj[:,0] = torch.from_numpy(ww1).to(device)
    skrew_traj[:,1] = torch.from_numpy(ww2).to(device)
    skrew_traj[:,2] = torch.from_numpy(ww3).to(device)
    skrew_traj[:,3] = torch.from_numpy(pp1).to(device)
    skrew_traj[:,4] = torch.from_numpy(pp2).to(device)
    skrew_traj[:,5] = torch.from_numpy(pp3).to(device)

    twist_traj[:,:3] = dw_f
    twist_traj[:,3] = torch.from_numpy(dp1_f).to(device)
    twist_traj[:,4] = torch.from_numpy(dp2_f).to(device)
    twist_traj[:,5] = torch.from_numpy(dp3_f).to(device)
    
    qtraj = skrew_traj
    return skrew_traj,twist_traj
...
def dim3_spline(xpoints, ypoints, zpoints, Vpoints, T, delta, num_timesteps):
    ...
    #### 5. xtraj and x_dot
    ptraj = torch.zeros(num_timesteps,3).to(device)
    pdot = torch.zeros(num_timesteps,3).to(device)

    ptraj[:, 0] = torch.from_numpy(pp1).to(device)
    ptraj[:, 1] = torch.from_numpy(pp2).to(device)
    ptraj[:, 2] = torch.from_numpy(pp3).to(device)

    pdot[:, 0] = torch.from_numpy(dp1_f).to(device)
    pdot[:, 1] = torch.from_numpy(dp2_f).to(device)
    pdot[:, 2] = torch.from_numpy(dp3_f).to(device)
...
def SE3_traj(traj_type_rot, traj_type_trans, *args, plot=True, **kargs):
    ...
    ptraj = ptraj.to(device)
    wtraj = wtraj.to(device)
    pdot = pdot.to(device)
    wdot = wdot.to(device)

    qtraj = torch.cat([wtraj, ptraj], dim=1).to(device)
    qdot = torch.cat([wdot, pdot], dim=1).to(device)
...
def traj_plot(qtraj, Xstable, plot_gap_ref = 1,  **kwargs):
    ...
    for i in range(num_timesteps):
        ...
        if i%plot_gap_ref == 0:
            ax.plot3D([pp[0],pp[0]+length*RR[0,0]], [pp[1],pp[1]+length*RR[1,0]], [pp[2],pp[2]+length*RR[2,0]], 'red')
            ax.plot3D([pp[0],pp[0]+length*RR[0,1]], [pp[1],pp[1]+length*RR[1,1]], [pp[2],pp[2]+length*RR[2,1]], 'green')
            ax.plot3D([pp[0],pp[0]+length*RR[0,2]], [pp[1],pp[1]+length*RR[1,2]], [pp[2],pp[2]+length*RR[2,2]], 'blue')
            
    ax.scatter(T_traj[0,0,3].to(device_c).detach().numpy(),T_traj[0,1,3].to(device_c).detach().numpy(),T_traj[0,2,3].to(device_c).detach().numpy(), marker="s",s=170,c='cyan')
    ax.scatter(Xstable[0,3].to(device_c).detach().numpy(),Xstable[1,3].to(device_c).detach().numpy(),Xstable[2,3].to(device_c).detach().numpy(), marker="o",s=170,c='blue')
    plt.show()
